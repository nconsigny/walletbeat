              ...
              if (entity.leaks && 'ref' in entity.leaks && entity.leaks.ref) {
                ...
                if (typeof ref === 'object' && !Array.isArray(ref)) {
                  if ('url' in ref && ref.url) {
                    const explanation = 'explanation' in ref && typeof ref.explanation === 'string'
                      ? ref.explanation
                ...
                // Handle array of references
                  ...
                  for (const item of ref) {
                    if (typeof item === 'string') {
                      ...
                      const url = item.url;
                      const explanation = 'explanation' in item && typeof item.explanation === 'string'
                        ? item.explanation
                        : `How ${entityName} handles multiple addresses`;
      ...
      if (features.privacy.dataCollection?.onchain?.ref) {
        ...
        if (typeof ref === 'object' && !Array.isArray(ref)) {
          if ('url' in ref && ref.url) {
            ...
            return [{
              urls: [{ url: ref.url, label: 'Onchain data collection' }] as NonEmptyArray<LabeledUrl>,
              explanation
            }];
          }
        }
        // Handle array of references
          ...
          for (const item of ref) {
            if (typeof item === 'string') {
              references.push({
                urls: [{ url: item, label: 'Onchain data collection' }] as NonEmptyArray<LabeledUrl>,
  ...
  switch (attributeId) {
        ...
        if (typeof ref === 'object' && !Array.isArray(ref) && ref !== null && 'url' in ref) {
          ...
          if (url) {
            return [{
              ...
              explanation: 'explanation' in ref ? String(ref.explanation) : 'Information about transaction submission options'
            }];
          }
        }
      }
    ...
    case 'selfHostedNode':
        ...
        if (typeof ref === 'object' && !Array.isArray(ref) && ref !== null && 'url' in ref) {
          ...
          if (url) {
            return [{
              ...
              explanation: 'explanation' in ref ? String(ref.explanation) : 'Information about self-hosted node support'
            }];
          }
        }
      }
      ...
      if (features.selfSovereignty.transactionSubmission?.l1?.selfBroadcastViaSelfHostedNode?.ref) {
        const ref = features.selfSovereignty.transactionSubmission.l1.selfBroadcastViaSelfHostedNode.ref;
        const urlsArray: LabeledUrl[] = [];
    ...
    case 'accountPortability':
        ...
        if (typeof ref === 'object' && !Array.isArray(ref) && ref !== null && 'url' in ref) {
          ...
          if (url) {
            return [{
              ...
              explanation: 'explanation' in ref ? String(ref.explanation) : 'Information about account portability features'
            }];
          }
        }
      }
      ...
      if (features.accountSupport?.rawErc4337?.ref || features.accountSupport?.eoa?.ref || features.accountSupport?.mpc?.ref) {
        ...
        if (typeof ref === 'object' && !Array.isArray(ref) && ref !== null && 'url' in ref) {
          ...
          if (url) {
            return [{
              ...
              explanation: 'explanation' in ref ? String(ref.explanation) : 'Information about transaction inclusion features'
            }];
          }
        }
      }
  ...
  switch (attributeId) {
        ...
        try {
          ...
          if (Array.isArray(ref)) {
            ref.forEach(refItem => {
              ...
              if (directUrl) {
                const label = typeof refItem === 'object' && 'explanation' in refItem 
              ...
              // Handle case where url is an array of {label, url} objects (Rabby case)
              else if (typeof refItem === 'object' && refItem !== null && 'url' in refItem && Array.isArray(refItem.url)) {
                // Process each URL in the array
                refItem.url.forEach(urlObj => {
                  if (typeof urlObj === 'object' && urlObj !== null && 'url' in urlObj) {
                    const url = urlObj.url;
                    const label = 'label' in urlObj && typeof urlObj.label === 'string'
                      ? urlObj.label
                      : 'Funding source';
                ...
                if (urlsArray.length > 0 && 'explanation' in refItem && typeof refItem.explanation === 'string') {
    ...
    case 'openSource':
      if (wallet.metadata.repoUrl && features.license) {
        ...
        const repoUrl = typeof wallet.metadata.repoUrl === 'string' 
        ...
        const urlsArray: LabeledUrl[] = [
          createLabeledUrl(
            licenseUrlPatterns[0],
            `${wallet.metadata.displayName} License File`
          )
        ];
        ...
        if (Array.isArray(ref)) {
          ref.forEach(urlItem => {
            ...
            if (typeof urlItem === 'object' && 'explanation' in urlItem && urlItem.explanation) {
              ...
              label = urlItem.explanation.slice(0, 30) + (urlItem.explanation.length > 30 ? '...' : '');
            }
            if (url) {
